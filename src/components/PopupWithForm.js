...
export default function PopupWithForm({ name, title, isOpen, onClose, children, onSubmit, formValid }) {
  return (
    <section className={`popup popup_type_${name} ${isOpen ? "popup_opened" : ''}`} >
      <div className={`popup__container popup__container_type_${name}`}>
        ...
        <form className={`popup__form popup__form_type_${name}`} name={name} onSubmit={onSubmit} noValidate>
          {children}
          <button className={`popup__submit ${formValid ? "" : "popup__submit_disabled"}`} disabled={formValid ? "" : true} type="submit">Сохранить</button>
        </form>
      </div>
    </section>
  )
